...
self.addEventListener('fetch',e=>{
  // firebasedatabase.app added alongside the legacy firebaseio.com domain —
  // both are live RTDB traffic (reads/writes/long-poll) and must never be
  // served from a stale cache. Previously only firebaseio.com was excluded;
  // this app's actual database URL uses firebasedatabase.app, so RTDB traffic
  // was falling through to the generic cache-first handler below. No evidence
  // this has caused a problem in practice (Firebase's long-poll transport
  // embeds session/sequence identifiers specifically to keep every request
  // URL unique, which is what would make this benign) — closed anyway since
  // the fix is one string and the failure mode, if it ever happened, would
  // look identical to the v5.26 stuck-sync bug and be very hard to trace back
  // here.
  if(['firebaseio.com','firebasedatabase.app','identitytoolkit'].some(d=>e.request.url.includes(d)))return;
